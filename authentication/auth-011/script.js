(function() {
  ...
  document.querySelectorAll('input, button, .toggle-password').forEach(el => { el.addEventListener('mouseenter', () => cursor.classList.add('hover')); el.addEventListener('mouseleave', () => cursor.classList.remove('hover')); });
  ...
  const card = document.getElementById('passwordCard'), glare = document.getElementById('cardGlare');
  ...
  const strengthLevels = [
    { score: 0, label: 'Too Weak', class: 'weak', icon: '🔴', title: 'Weak Password', desc: 'Add more variety to make it stronger', crack: 'Instantly' },
    { score: 1, label: 'Weak', class: 'weak', icon: '🟠', title: 'Weak Password', desc: 'Add uppercase, numbers, or symbols', crack: 'Seconds' },
    { score: 2, label: 'Fair', class: 'fair', icon: '🟡', title: 'Fair Password', desc: 'Getting better! Add more characters', crack: 'Minutes' },
    { score: 3, label: 'Good', class: 'good', icon: '🟢', title: 'Good Password', desc: 'Almost there! Add special characters', crack: 'Hours' },
    { score: 4, label: 'Strong', class: 'strong', icon: '🛡️', title: 'Strong Password', desc: 'Excellent! Your password is secure', crack: 'Years' },
    { score: 5, label: 'Very Strong', class: 'strong', icon: '🔐', title: 'Very Strong Password', desc: 'Maximum security achieved!', crack: 'Centuries' }
  ];
  ...
  function evaluatePassword(password) {
    ...
    return { score: Math.min(5, score), rules };
  }

  function updateStrengthUI(result) {
    const { score, rules } = result;
    ...
    // Update detail card
    detailIcon.textContent = level.icon;
    detailTitle.textContent = level.title;
    detailDesc.textContent = level.desc;
    
    // Update crack time
    crackValue.textContent = level.crack;
    crackValue.style.color = score >= 4 ? 'var(--success)' : score >= 2 ? 'var(--warning)' : 'var(--danger)';
    
    // Update requirements
    if (rules) {
      requirements.forEach(req => {
        const rule = req.dataset.rule;
        const valid = rules[rule];
        req.classList.toggle('valid', valid);
        req.classList.toggle('invalid', !valid && passwordInput.value.length > 0);
  ...
  console.log('🔷 UIXForge — AUTH-011 Password Strength Indicator Ready');
  console.log('📦 Part of 200+ Production-Ready Components');
  console.log('🔐 5 Rules + 6 Strength Levels + Crack Time Estimator');
})();